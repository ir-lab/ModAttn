...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target_1, target_2, joint_angles, ee_pos, ee_traj, ee_xy, length, target_1_pos, target_2_pos, phis, mask, target_1_xy, target_2_xy, sentence, joint_angles_traj, displacement_1, displacement_2) in enumerate(data_loader):
        ...
        if stage >= 1:
            ...
            writer.add_scalar('train/loss_tar_1_pos', loss01.item(), global_step=epoch_idx * len(data_loader) + idx)
            writer.add_scalar('train/loss_tar_2_pos', loss02.item(), global_step=epoch_idx * len(data_loader) + idx)
            writer.add_scalar('train/displacement_1', loss11.item(), global_step=epoch_idx * len(data_loader) + idx)
            writer.add_scalar('train/displacement_2', loss12.item(), global_step=epoch_idx * len(data_loader) + idx)
            writer.add_scalar('train/loss_ee_pos', loss2.item(), global_step=epoch_idx * len(data_loader) + idx)
        ...
        if stage >= 2:
            ...
            writer.add_scalar('train/loss_traj', loss4.item(), global_step=epoch_idx * len(data_loader) + idx)
            loss = loss + loss4
...
if __name__ == '__main__':
    name = 'train-rgb-sub-attn-abs-action-separate-tar2-nets-vision-embed-2-stage-2'
