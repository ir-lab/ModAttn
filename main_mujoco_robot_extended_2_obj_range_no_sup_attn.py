...
from utils.load_data_rgb_abs_action_dalle_2_obj_fov40 import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def step(data, model, optimizer, criterion, criterion2, stage, writer, epoch_idx, idx, len_data_loader):
    ...
    loss_target_2_pos_attn = range_supervised_attn_loss(target_2_xy, attn_map2, 5, criterion)
...
def train_2(writer, name, epoch_idx, data_loader, data_loader_dmp, model, 
    ...
    for data2 in data_loader_dmp:
        ...
        global_step = epoch_idx * len(data_loader_dmp) + idx
...
def main(batch_size=96):
    ...
    parser.add_argument('--name', default='train-rgb-sub-attn-abs-action-vision-embed-3-stage-dalle-2-obj-fov40-old-curri')
