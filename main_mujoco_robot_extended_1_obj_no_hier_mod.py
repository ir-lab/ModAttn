...
from utils.load_data_rgb_abs_action_dalle_1_obj_fov40 import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def range_supervised_attn_loss(obj_xy, attn_map, slot_idx, criterion, attn_map_offset=5):
    loss = 0
    img_attn = 0
    for i in [-1, 0, 1]:
        for j in [-1, 0, 1]:
            ...
            attn_index = pixel_position_to_attn_index(new_xy, attn_map_offset=attn_map_offset)
...
def main(batch_size=96):
    ...
    parser.add_argument('--name', default='train-rgb-sub-attn-abs-action-vision-embed-no-stage-dalle-1-obj-fov40')
    ...
    # load model
    model = Backbone(img_size=224, embedding_size=192, input_nc=3, num_traces_in=7, num_traces_out=10, num_weight_points=12, device=device).to(device)
