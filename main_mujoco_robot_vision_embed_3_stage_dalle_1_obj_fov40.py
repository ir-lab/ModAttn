...
from models.backbone_rgbd_sub_attn_vision_embed_dalle_1_obj import Backbone
from utils.load_data_rgb_abs_action_fast_gripper_finetuned_attn_vision_embed_dalle_1_obj_fov40 import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def step(data, model, optimizer, criterion, criterion2, stage, writer, epoch_idx, idx, len_data_loader):
    img, target_1, joint_angles, ee_pos, ee_traj, ee_xy, length, target_1_pos, phis, mask, target_1_xy, sentence, joint_angles_traj, displacement_1 = data
    img = img.to(device)
    ...
    phis = phis.to(device)
    mask = mask.to(device)
    attn_index_tar_1 = pixel_position_to_attn_index(target_1_xy, attn_map_offset=5)
    ...
    if stage == 0:
        ...
        target_1_position_pred, ee_pos_pred, displacement_1_pred, attn_map, attn_map2, attn_map3 = model(img, joint_angles, sentence, phis, stage)
    else:
        target_1_position_pred, ee_pos_pred, displacement_1_pred, attn_map, attn_map2, attn_map3, attn_map4, trajectory_pred = model(img, joint_angles, sentence, phis, stage)
    ...
    if stage >= 1:
        ...
        if (epoch_idx * len_data_loader + idx) % 100 == 0 or (epoch_idx * len_data_loader + idx + 1) % 100 == 0:
    ...
    if stage == 0:
        ...
        print(f'epoch {epoch_idx}, step {idx}/{len_data_loader}, stage {stage}, l_all {loss.item():.2f}, l0 {(loss01).item():.2f}, l1 {(loss11).item():.2f}, l2 {loss2.item():.2f}')
    else:
        print(f'epoch {epoch_idx}, step {idx}/{len_data_loader}, stage {stage}, l_all {loss.item():.2f}, l0 {(loss01).item():.2f}, l1 {(loss11).item():.2f}, l2 {loss2.item():.2f}, l4 {loss4.item():.2f}')
...
def train_1(writer, name, epoch_idx, data_loader, model, 
    ...
    for data1 in data_loader:
        ...
        optimizer.zero_grad()
        loss1 = step(data1, model, optimizer, criterion, criterion2, stage, writer, epoch_idx, idx, len(data_loader))
        idx = idx + 1
        ...
        if save_ckpt:
            if not os.path.isdir(os.path.join(ckpt_path, name)):
                ...
                os.system(f'mkdir -p {os.path.join(ckpt_path, name)}')
...
def train_2(writer, name, epoch_idx, data_loader, data_loader_dmp, model, 
    optimizer, scheduler, criterion, ckpt_path, save_ckpt, stage,
    print_attention_map=False, curriculum_learning=False, supervised_attn=False):
    model.train()
    criterion2 = nn.L1Loss(reduction='none')
    ...
    for data1, data2 in zip(data_loader, data_loader_dmp):
        ...
        optimizer.zero_grad()
        loss1 = step(data1, model, optimizer, criterion, criterion2, 1, writer, epoch_idx, idx, len(data_loader) * 2)
        idx = idx + 1
...
def test(writer, name, epoch_idx, data_loader, model, criterion, train_dataset_size, stage, print_attention_map=False, train_split=False):
    with torch.no_grad():
        ...
        for idx, (img, target_1, joint_angles, ee_pos, ee_traj, ee_xy, length, target_1_pos, phis, mask, target_1_xy, sentence, joint_angles_traj, displacement_1) in enumerate(data_loader):
            global_step = epoch_idx * len(data_loader) + idx
            ...
            if stage >= 1:
                print(idx, f'err tar 1 pos: {(error_target_1_position / num_datapoints).item():.4f} err ee pos: {(error_ee_pos / num_datapoints).item():.4f} err displace 1: {(error_displacement_1 / num_datapoints).item():.4f}')
            if stage >= 2:
...
def main(batch_size=96):
    ...
    parser.add_argument('--name', default='train-rgb-sub-attn-abs-action-vision-embed-3-stage-dalle-1-obj-fov40')
    ...
    # dataset_test = DMPDatasetEERandTarXYLang(val_dirs, random=True, length_total=120,
    #     source_root = data_source_root,
    #     target_root = data_target_root)
    ...
    for i in range(0, 300):
        whether_test = ((i % 5) == 0)
        if loss_stage <= 1:
            loss_stage = train_1(writer, name, i, data_loader_train, model, optimizer, scheduler,
            ...
            loss_stage = train_2(writer, name, i, data_loader_train, data_loader_train_dmp, model, optimizer, scheduler,
                criterion, ckpt_path, save_ckpt, loss_stage, supervised_attn=supervised_attn, curriculum_learning=curriculum_learning, print_attention_map=False)
        if whether_test:
